...
class OrderLine( Message ):
    def __init__( self, article_code=None, amount=None ):
        self.next_message= ['1020' #OrderLine
                            ,'1030' #OrderLineBonus
                            ,'1050' #OrderClose
                            ]
...
#        self.article_code = re.sub("^0+","",msg[4:17])
        self.article_code = msg[4:17]

        self.article_national_code = msg[10:17]
        #control digit
        print ":",msg[17:21]
        self.amount =  msg[17:21]

    def __str__( self ):
        return messages['ORDER_LINE_CODE'] +  messages['ORDER_LINE_SUBCODE']  + self.article_code + str(self.amount).rjust(4,'0') + messages['END_MESSAGE']
